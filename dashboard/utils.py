...
import json
from .models import Nutrition,NutritionItems,MentalHealth
...
def MentalSocre(request):
    if request.method == 'POST':
        data = json.loads(request.body)
        score = data['score']
        MentalHealth.objects.create(
            user=request.user,
            score=score
        )
        return JsonResponse({'message': 'Mental score saved successfully.'}, status=200)
