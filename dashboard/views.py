...
from .models import TdeeData,MentalHealth
...
def Reports(request):
    data = MentalHealth.objects.filter(user=request.user).values('score','date')
    data2 = TdeeData.objects.filter(user=request.user).values('created_at')
    for entry in data2:
        tdee_data_instance = TdeeData.objects.get(user=request.user, created_at=entry['created_at'])
    entry['calories'] = tdee_data_instance.calculateTdee()
    context = {
        'data':json.dumps(list(data),cls=DjangoJSONEncoder),
        'data2':json.dumps(list(data2),cls=DjangoJSONEncoder),
    }
